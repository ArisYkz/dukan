-- ==============================================
-- DUKAN FULL SCHEMA + SEED — v2.1 (storage skipped)
-- generated 2026-08-26
-- Open THIS file: supabase/apply-full-schema-v2.sql
-- Paste the ENTIRE file into the Supabase SQL Editor and Run once.
-- Resets public schema, applies all migrations + demo seed.
-- Storage-schema statements are skipped (plain NOTICE) because the
-- SQL-editor role usually can't access storage; image upload is a
-- separate follow-up once you're in.
-- ==============================================
...
DO $$ BEGIN RAISE NOTICE 'skipping storage statement (storage schema not accessible)'; END $$;
...
----------------------------------------------
-- MIGRATION: 20260501000000_restore_verification.sql
----------------------------------------------
...
--     policies below and by 20260506000001_audit_fixes (RLS enable).
--   - stores.verification_status: returned by get_public_store and updated by
--     the admin-verify edge function.
...
-- CONCURRENTLY removed: this script runs in a single transaction in the SQL editor
-- (and rebuilds a fresh schema, so there is no concurrent traffic to protect).
...
CREATE INDEX IF NOT EXISTS idx_order_contacts_store_id ON public.order_contacts(store_id);


CREATE INDEX IF NOT EXISTS idx_store_promo_codes_store_id ON public.store_promo_codes(store_id);
